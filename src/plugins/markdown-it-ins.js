function markdownItIns(md) {
  ...
  md.inline.ruler.push('ins', (state, silent) => {
    let start = state.pos;
    ...
    // Check if we have a double plus
    if (marker !== 0x2b /* + */) {
      return false;
    }
    ...
    while (pos + 2 < max) {
      if (
        ...
        // let content = state.src.slice(start + 2, pos);
  ...
  md.block.ruler.before(
    ...
    (state, startLine, endLine, silent) => {
      ...
      // Check if we have a double plus at the start
      if (state.src.charCodeAt(pos) !== 0x2b /* + */) {
        return false;
      }
      if (state.src.charCodeAt(pos + 1) !== 0x2b /* + */) {
        return false;
      }
      ...
      for (let line = startLine; line <= endLine; line++) {
        ...
        for (let i = lineStart; i + 1 < lineMax; i++) {
          if (
            state.src.charCodeAt(i) === 0x2b /* + */ &&
            state.src.charCodeAt(i + 1) === 0x2b /* + */
          ) {
            endPos = i;
            break;
          }
